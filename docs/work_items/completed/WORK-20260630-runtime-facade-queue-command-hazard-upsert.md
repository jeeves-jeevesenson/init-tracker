# WORK-20260630-runtime-facade-queue-command-hazard-upsert: Queue-backed facade command for map hazard upsert
...
Route only `POST /api/dm/map/hazards` through `ServerRuntimeFacade.submit_command(...)` and the existing Tk/LanController queue adapter seam, preserving existing map hazard create/update behavior.

## Scope

- Added `COMMAND_UPSERT_MAP_HAZARD = "upsert_map_hazard"` constant to `server_runtime.py`.
- Routed `COMMAND_UPSERT_MAP_HAZARD` in `ServerRuntimeFacade.submit_command` via the queue adapter seam.
- Added dispatch logic for `typ == "upsert_map_hazard"` in `_lan_apply_action` in `dnd_initative_tracker.py` on the Tk/main thread.
- Updated `POST /api/dm/map/hazards` in `dnd_initative_tracker.py` to delegate to the facade.
- Added focused unit tests in `tests/test_server_runtime.py` for map hazard upsert command execution, validation errors, and route mapping.

## Non-Goals / Forbidden Scope

- Did not migrate DELETE /api/dm/map/hazards/{hazard_id}.
...
- `.venv/bin/python -m unittest tests/test_server_runtime.py` (48/48 tests passed)
