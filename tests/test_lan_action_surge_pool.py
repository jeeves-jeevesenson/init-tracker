...
class LanActionSurgePoolTests(unittest.TestCase):
    ...
    def test_action_surge_use_rejects_when_pool_exhausted(self):
        toasts = []
        app = object.__new__(tracker_mod.InitiativeTracker)
        app._oplog = lambda *args, **kwargs: None
        app._is_admin_token_valid = lambda token: False
        ...
        app._profile_for_player_name = lambda name: {"leveling": {"classes": [{"name": "Fighter", "level": 10}]}}
        app._fighter_level_from_profile = lambda profile: 10
        app._consume_resource_pool_for_cast = lambda player_name, pool_id, cost: (False, "No Action Surge uses remain, matey.")
        app._log = lambda message, cid=None: None
        app._rebuild_table = lambda scroll_to_current=True: None
        app.in_combat = True
        app.current_cid = 1
        app.round_num = 1
        ...
        self.assertEqual(app.combatants[1].action_remaining, 0)
        self.assertEqual(app.combatants[1].action_total, 1)
        self.assertIn((42, "No Action Surge uses remain, matey."), toasts)

    def test_action_surge_use_rejects_for_low_level_fighter(self):
        toasts = []
        app = object.__new__(tracker_mod.InitiativeTracker)
        app._oplog = lambda *args, **kwargs: None
        app._is_admin_token_valid = lambda token: False
        ...
        app._is_valid_summon_turn_for_controller = lambda controlling, target, current: True
        app._pc_name_for = lambda cid: "John Twilight"
        app._profile_for_player_name = lambda name: {"leveling": {"classes": [{"name": "Fighter", "level": 1}]}}
        app._fighter_level_from_profile = lambda profile: 1
        app._consume_resource_pool_for_cast = lambda player_name, pool_id, cost: (True, "")
        app._log = lambda message, cid=None: None
        app._rebuild_table = lambda scroll_to_current=True: None
        app.in_combat = True
        app.current_cid = 1
        ...
        self.assertEqual(app.combatants[1].action_remaining, 0)
        self.assertIn((42, "Only fighters level 2+ can use Action Surge, matey."), toasts)

    def test_action_surge_use_handles_numbered_combatant_name(self):
        consumed = []
        app = object.__new__(tracker_mod.InitiativeTracker)
        app._oplog = lambda *args, **kwargs: None
        app._is_admin_token_valid = lambda token: False
        ...
        app._strip_combat_name_suffix = tracker_mod.InitiativeTracker._strip_combat_name_suffix

        # Resolver returns the numbered name as it would in combat
        app._pc_name_for = lambda cid: "John Twilight 1"

        # Profile lookup should receive the STRIPPED name
        app._profile_for_player_name = lambda name: {"leveling": {"classes": [{"name": "Fighter", "level": 10}]}} if name == "John Twilight" else None
        app._fighter_level_from_profile = lambda profile: 10
        app._consume_resource_pool_for_cast = lambda player_name, pool_id, cost: (consumed.append((player_name, pool_id, cost)) or True, "")
        app._log = lambda message, cid=None: None
        app._rebuild_table = lambda scroll_to_current=True: None
