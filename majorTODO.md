...
- `_lan_apply_action()` delegates `attack_request`, `spell_target_request`, `reaction_response`, `end_turn`, `move`, `cycle_movement_mode`, `perform_action`, `aoe_move`, `aoe_remove`, `manual_override_hp`, `manual_override_spell_slot`, `manual_override_resource_pool`, `reaction_prefs_update`, `mount_request`, `mount_response`, `dismount`, `dash`, `use_action`, `use_bonus_action`, `stand_up`, `reset_turn`, `lay_on_hands_use`, `inventory_adjust_consumable`, `use_consumable`, `second_wind_use`, `action_surge_use`, `star_advantage_use`, `monk_patient_defense`, `monk_step_of_wind`, `monk_elemental_attunement`, `monk_elemental_burst`, and `monk_uncanny_metabolism` through the service; movement/action, AoE manipulation, turn-local/mobility-lite, and fighter/monk resource-actions now flow through family dispatcher branches keyed by `MOVEMENT_ACTION_COMMAND_TYPES`, `AOE_MANIPULATION_COMMAND_TYPES`, `TURN_LOCAL_COMMAND_TYPES`, and `FIGHTER_MONK_RESOURCE_ACTION_TYPES`, and the former ~2600 lines of inline combat adjudication now live in named `InitiativeTracker._adjudicate_attack_request`, `_adjudicate_spell_target_request`, and `_adjudicate_reaction_response` methods.
...
- The LAN player client is already feature-rich, including attacks, spell targeting, reactions, inventory, condition icons, token portraits, reconnect recovery, and notifications. Authoritative action processing for combat plus the migrated movement/action, AoE manipulation, wild-shape, resource/consumable, turn-local, fighter/monk action, and spell-launch slices (`attack_request`, `spell_target_request`, `reaction_response`, `end_turn`, `move`, `cycle_movement_mode`, `perform_action`, `aoe_move`, `aoe_remove`, `wild_shape_apply`, `wild_shape_pool_set_current`, `wild_shape_revert`, `wild_shape_regain_use`, `wild_shape_regain_spell`, `wild_shape_set_known`, `manual_override_hp`, `manual_override_spell_slot`, `manual_override_resource_pool`, `reaction_prefs_update`, `mount_request`, `mount_response`, `dismount`, `dash`, `use_action`, `use_bonus_action`, `stand_up`, `reset_turn`, `lay_on_hands_use`, `inventory_adjust_consumable`, `use_consumable`, `second_wind_use`, `action_surge_use`, `star_advantage_use`, `monk_patient_defense`, `monk_step_of_wind`, `monk_elemental_attunement`, `monk_elemental_burst`, `monk_uncanny_metabolism`, `cast_spell`, `cast_aoe`) now enters through `PlayerCommandService`; movement/action, AoE manipulation, wild-shape, turn-local/mobility-lite, fighter/monk resource-actions, and spell-launch route through family dispatch branches in `_lan_apply_action()`, and the deep combat/cast rules logic lives in `_adjudicate_*` / `_handle_cast_*_request` tracker methods. Other player commands (summon/echo actions, bard/glamour specialty actions, etc.) still resolve inline in `_lan_apply_action()`. The older `cast_aoe_adjust` reference appears stale on this branch; no live handler or test coverage for it currently exists.
...
Remaining coupling:
- `_ACTION_MESSAGE_TYPES` is large. `_lan_apply_action()` still dispatches a few non-combat player actions inline (echo/summon, bard/glamour specialty resolves, etc.), though combat-adjudicating plus migrated movement/action, AoE manipulation, wild-shape, resource/consumable, turn-local, fighter/monk, and spell-launch slices (`attack_request`, `spell_target_request`, `reaction_response`, `end_turn`, `move`, `cycle_movement_mode`, `perform_action`, `aoe_move`, `aoe_remove`, `wild_shape_apply`, `wild_shape_pool_set_current`, `wild_shape_revert`, `wild_shape_regain_use`, `wild_shape_regain_spell`, `wild_shape_set_known`, `manual_override_hp`, `manual_override_spell_slot`, `manual_override_resource_pool`, `reaction_prefs_update`, `mount_request`, `mount_response`, `dismount`, `dash`, `use_action`, `use_bonus_action`, `stand_up`, `reset_turn`, `lay_on_hands_use`, `inventory_adjust_consumable`, `use_consumable`, `second_wind_use`, `action_surge_use`, `star_advantage_use`, `monk_patient_defense`, `monk_step_of_wind`, `monk_elemental_attunement`, `monk_elemental_burst`, `monk_uncanny_metabolism`, `cast_spell`, `cast_aoe`) now route through `PlayerCommandService`.
...
Confirmed leverage:
- `player_command_service.py` owns envelope logic for migrated player combat plus movement/action, AoE manipulation, wild-shape, resource/consumable, turn-local, self-state, and spell-launch commands: turn/claim validation, pending-reaction attacker gate, reactor-cid match, move/mobility/action-economy orchestration, AoE manipulation request dispatch, wild-shape/YAML/resource orchestration, manual override/resource/inventory orchestration, spell-launch request dispatch, and `CombatService` manual-override dispatch/fallback mutation.
...
- `_lan_apply_action()` now routes the movement/action family (`typ in MOVEMENT_ACTION_COMMAND_TYPES` → `dispatch_movement_action_command(...)`), the bounded AoE manipulation family (`typ in AOE_MANIPULATION_COMMAND_TYPES` → `dispatch_aoe_manipulation_command(...)`), the bounded wild-shape family (`typ in WILD_SHAPE_COMMAND_TYPES` → `dispatch_wild_shape_command(...)`), and the adjacent turn-local/mobility-lite family (`typ in TURN_LOCAL_COMMAND_TYPES` → `dispatch_turn_local_command(...)`) instead of keeping those branches inline.
...
- hellish rebuke still uses a follow-up `hellish_rebuke_resolve` command path; it now reads canonical prompt resolution metadata, but the broader prompt family is not yet fully collapsed into one generic handler.
...
Current state: the LAN client is feature-rich and battle-tested in focused areas. The combat-adjudicating player command slice now has explicit contracts, a canonical prompt model, reconnect-safe prompt snapshots, and service-dispatched resume handling, and the adjacent movement/action, AoE manipulation, wild-shape, turn-local/mobility-lite, fighter/monk resource-action, and spell-launch (`cast_spell` / `cast_aoe`) families now dispatch through `PlayerCommandService` via shared family routers; `_lan_apply_action()` still owns summon/echo plus the remaining inline bard/glamour and other specialty resolves. The old `cast_aoe_adjust` TODO reference is stale on this branch.

Next likely major pass: extract the remaining inline bard/glamour specialty player-command family (`command_resolve`, `bardic_inspiration_grant`, `bardic_inspiration_use`, `mantle_of_inspiration`, `beguiling_magic_restore`, `beguiling_magic_use`) out of `_lan_apply_action()` without reopening the already-migrated combat/prompt, AoE manipulation, wild-shape, or spell-launch branches.
...
- Completed major passes:
  ...
  - **Bounded spell-launch command family now dispatches through `PlayerCommandService` (2026-04-17):**
    ...
    - summon/echo spawning and specialty resolves remained inline as deliberate out-of-scope for that pass; the bounded AoE manipulation family landed immediately afterward.
...
- In-progress pass: `None. The bounded AoE manipulation extraction pass (aoe_move / aoe_remove) has landed.`
- Next recommended pass: `Continue extending backend authority: extract the remaining inline bard/glamour specialty player-command family (command_resolve, bardic_inspiration_grant, bardic_inspiration_use, mantle_of_inspiration, beguiling_magic_restore, beguiling_magic_use) out of _lan_apply_action() without reopening the migrated combat/prompt, AoE manipulation, wild-shape, or spell-launch branches.`
- Blocked items:
...
### Extract the remaining inline bard/glamour specialty family out of `_lan_apply_action()`

The bounded AoE manipulation (`aoe_move` / `aoe_remove`) extraction pass landed on 2026-04-17. The next clean authority target is the remaining inline bard/glamour specialty slice, which is still player-facing, cohesive, and materially smaller-risk than reopening summon/echo ownership.

Why this is next:
- `_lan_apply_action()` still owns a contiguous specialty player-command hotspot around `command_resolve`, `bardic_inspiration_grant`, `bardic_inspiration_use`, `mantle_of_inspiration`, `beguiling_magic_restore`, and `beguiling_magic_use`.
- That family is still heavily player-command oriented: it spends player resources/actions, builds player-facing toasts/logging, and mutates combat state without needing a broad summon or map-window rewrite.
- Moving it behind `PlayerCommandService` continues the same backend-envelope pattern without reopening the already-migrated combat, prompt, movement, AoE manipulation, wild-shape, turn-local, or spell-launch slices.

Recommended scope:
- extract the bounded bard/glamour specialty request family behind `PlayerCommandService`, starting with `command_resolve`, `bardic_inspiration_grant`, `bardic_inspiration_use`, `mantle_of_inspiration`, `beguiling_magic_restore`, and `beguiling_magic_use`.
- add explicit request contracts in `player_command_contracts.py` for the extracted family.
- keep the already-migrated combat + prompt + movement/action + AoE manipulation + wild-shape + turn-local/resource/fighter/monk + spell-launch command boundaries stable.
- continue to avoid broad summon, echo, or map-window rewrites in the same pass.

Recommended validation:
- contract coverage:
  - `tests/test_player_command_contracts.py`
  - `tests/test_lan_action_message_types_allowlist.py`
- targeted specialty-command coverage (`tests/test_beguiling_magic.py`, `tests/test_mantle_of_inspiration.py`, and adjacent bard/reaction suites).
- adjacent player-command service seam regression coverage.

If this pass lands cleanly, `_lan_apply_action()` will lose another contiguous player-command hotspot and the backend authority surface will keep advancing before the higher-risk summon/echo ownership work.
