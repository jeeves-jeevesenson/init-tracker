...
- **DM map loading and workspace subscription hotfix landed 2026-05-23**: restores `/dm/map` loading and mutations without requiring `INIT_TRACKER_ENABLE_TACTICAL_MAP=1`. Standard combat loops remain lightweight and combat-lite, while explicit map routes (`/dm/map`, `/api/dm/map/*`) force tactical map rendering. Real-time updates via `/ws/dm` use workspace query parameter routing and JSON `subscribe_map` subscriptions to deliver tactical payloads dynamically only to active map-subscribed DM screens, protecting dashboard performance. Tested with 9 new integration tests in `test_dm_tactical_map_routes.py`.
