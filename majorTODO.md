...
- `_lan_apply_action()` delegates `attack_request`, `spell_target_request`, `reaction_response`, `end_turn`, `move`, `cycle_movement_mode`, `perform_action`, `aoe_move`, `aoe_remove`, `manual_override_hp`, `manual_override_spell_slot`, `manual_override_resource_pool`, `reaction_prefs_update`, `mount_request`, `mount_response`, `dismount`, `dash`, `use_action`, `use_bonus_action`, `stand_up`, `reset_turn`, `lay_on_hands_use`, `inventory_adjust_consumable`, `use_consumable`, `second_wind_use`, `action_surge_use`, `star_advantage_use`, `monk_patient_defense`, `monk_step_of_wind`, `monk_elemental_attunement`, `monk_elemental_burst`, `monk_uncanny_metabolism`, `echo_summon`, `echo_swap`, `dismiss_summons`, `dismiss_persistent_summon`, `reappear_persistent_summon`, `assign_pre_summon`, and `echo_tether_response` through the service; movement/action, AoE manipulation, turn-local/mobility-lite, fighter/monk resource-actions, and summon/echo specialty commands now flow through family dispatcher branches keyed by `MOVEMENT_ACTION_COMMAND_TYPES`, `AOE_MANIPULATION_COMMAND_TYPES`, `TURN_LOCAL_COMMAND_TYPES`, `FIGHTER_MONK_RESOURCE_ACTION_TYPES`, and `SUMMON_ECHO_SPECIALTY_COMMAND_TYPES`, and the former ~2600 lines of inline combat adjudication now live in named `InitiativeTracker._adjudicate_attack_request`, `_adjudicate_spell_target_request`, and `_adjudicate_reaction_response` methods.
...
- The LAN player client is already feature-rich, including attacks, spell targeting, reactions, inventory, condition icons, token portraits, reconnect recovery, and notifications. Authoritative action processing for combat plus the migrated movement/action, AoE manipulation, wild-shape, resource/consumable, turn-local, fighter/monk action, spell-launch, bard/glamour specialty, and summon/echo specialty slices (`attack_request`, `spell_target_request`, `reaction_response`, `end_turn`, `move`, `cycle_movement_mode`, `perform_action`, `aoe_move`, `aoe_remove`, `wild_shape_apply`, `wild_shape_pool_set_current`, `wild_shape_revert`, `wild_shape_regain_use`, `wild_shape_regain_spell`, `wild_shape_set_known`, `manual_override_hp`, `manual_override_spell_slot`, `manual_override_resource_pool`, `reaction_prefs_update`, `mount_request`, `mount_response`, `dismount`, `dash`, `use_action`, `use_bonus_action`, `stand_up`, `reset_turn`, `lay_on_hands_use`, `inventory_adjust_consumable`, `use_consumable`, `second_wind_use`, `action_surge_use`, `star_advantage_use`, `monk_patient_defense`, `monk_step_of_wind`, `monk_elemental_attunement`, `monk_elemental_burst`, `monk_uncanny_metabolism`, `cast_spell`, `cast_aoe`, `command_resolve`, `bardic_inspiration_grant`, `bardic_inspiration_use`, `mantle_of_inspiration`, `beguiling_magic_restore`, `beguiling_magic_use`, `echo_summon`, `echo_swap`, `dismiss_summons`, `dismiss_persistent_summon`, `reappear_persistent_summon`, `assign_pre_summon`, `echo_tether_response`) now enters through `PlayerCommandService`; movement/action, AoE manipulation, wild-shape, turn-local/mobility-lite, fighter/monk resource-actions, spell-launch, bard/glamour specialty, and summon/echo specialty routes dispatch through family branches in `_lan_apply_action()`, and the deep combat/cast/specialty rules logic lives in `_adjudicate_*` / `_handle_*_request` tracker methods. The largest remaining inline player-command specialty resolves are now `initiative_roll` and `hellish_rebuke_resolve`. The older `cast_aoe_adjust` reference appears stale on this branch; no live handler or test coverage for it currently exists.
...
Remaining coupling:
- `_ACTION_MESSAGE_TYPES` is large. `_lan_apply_action()` still dispatches a few non-migrated specialty actions inline (notably `initiative_roll` and `hellish_rebuke_resolve`), though combat-adjudicating plus migrated movement/action, AoE manipulation, wild-shape, resource/consumable, turn-local, fighter/monk, spell-launch, bard/glamour specialty, and summon/echo specialty slices (`attack_request`, `spell_target_request`, `reaction_response`, `end_turn`, `move`, `cycle_movement_mode`, `perform_action`, `aoe_move`, `aoe_remove`, `wild_shape_apply`, `wild_shape_pool_set_current`, `wild_shape_revert`, `wild_shape_regain_use`, `wild_shape_regain_spell`, `wild_shape_set_known`, `manual_override_hp`, `manual_override_spell_slot`, `manual_override_resource_pool`, `reaction_prefs_update`, `mount_request`, `mount_response`, `dismount`, `dash`, `use_action`, `use_bonus_action`, `stand_up`, `reset_turn`, `lay_on_hands_use`, `inventory_adjust_consumable`, `use_consumable`, `second_wind_use`, `action_surge_use`, `star_advantage_use`, `monk_patient_defense`, `monk_step_of_wind`, `monk_elemental_attunement`, `monk_elemental_burst`, `monk_uncanny_metabolism`, `cast_spell`, `cast_aoe`, `command_resolve`, `bardic_inspiration_grant`, `bardic_inspiration_use`, `mantle_of_inspiration`, `beguiling_magic_restore`, `beguiling_magic_use`, `echo_summon`, `echo_swap`, `dismiss_summons`, `dismiss_persistent_summon`, `reappear_persistent_summon`, `assign_pre_summon`, `echo_tether_response`) now route through `PlayerCommandService`.
...
Confirmed leverage:
- `player_command_service.py` owns envelope logic for migrated player combat plus movement/action, AoE manipulation, wild-shape, resource/consumable, turn-local, self-state, spell-launch, bard/glamour specialty, and summon/echo specialty commands: turn/claim validation, pending-reaction attacker gate, reactor-cid match, move/mobility/action-economy orchestration, AoE manipulation request dispatch, wild-shape/YAML/resource orchestration, manual override/resource/inventory orchestration, spell-launch request dispatch, specialty dispatch, and `CombatService` manual-override dispatch/fallback mutation.
...
- `_lan_apply_action()` now routes movement/action, AoE manipulation, wild-shape, turn-local/mobility-lite, spell-launch, bard/glamour specialty, and summon/echo specialty families through `PlayerCommandService` family dispatchers instead of keeping those branches inline.
...
Current state: the LAN client is feature-rich and battle-tested in focused areas. The combat-adjudicating player command slice now has explicit contracts, a canonical prompt model, reconnect-safe prompt snapshots, and service-dispatched resume handling, and the adjacent movement/action, AoE manipulation, wild-shape, turn-local/mobility-lite, fighter/monk resource-action, spell-launch (`cast_spell` / `cast_aoe`), bard/glamour specialty, and summon/echo specialty families now dispatch through `PlayerCommandService` via shared family routers; `_lan_apply_action()` still owns a few non-migrated specialty resolves (notably `initiative_roll` and `hellish_rebuke_resolve`). The old `cast_aoe_adjust` TODO reference is stale on this branch.

Next likely major pass: extract the remaining inline initiative/reaction specialty resolves (`initiative_roll` and `hellish_rebuke_resolve`) out of `_lan_apply_action()` behind the same service-envelope + tracker-helper dispatch seam.
...
- In-progress pass: `None. The bounded summon/echo specialty extraction pass has landed.`
- Next recommended pass: `Continue extending backend authority: extract the remaining inline initiative/reaction specialty resolves (initiative_roll + hellish_rebuke_resolve) out of _lan_apply_action() behind the same thin service-envelope + tracker-helper dispatch pattern.`
- Blocked items:
...
### Extract the remaining inline initiative/reaction specialty resolves out of `_lan_apply_action()`

The bounded summon/echo specialty extraction pass landed on 2026-04-18. The next clean authority target is the remaining inline initiative/reaction specialty slice (`initiative_roll` and `hellish_rebuke_resolve`) that still co-locates transport checks and adjudication in `_lan_apply_action()`.

Why this is next:
- `_lan_apply_action()` still owns the full `initiative_roll` and `hellish_rebuke_resolve` branches end-to-end.
- Those branches are now the largest remaining player-command authority hotspot after combat/prompt, movement/action, turn-local, wild-shape, fighter/monk, spell-launch, AoE manipulation, bard/glamour specialty, and summon/echo specialty extraction.
- Moving these branches behind `PlayerCommandService` continues backend authority migration without broad DM-side or map-rendering rewrites.

Recommended scope:
- extract `initiative_roll` and `hellish_rebuke_resolve` through `PlayerCommandService` family dispatch.
- keep already-migrated command families stable.
- preserve existing prompt-resolution, slot-spend, KO cleanup, and broadcast/toast behavior while shrinking `_lan_apply_action()` ownership.

Recommended validation:
- service/dispatch coverage for the extracted initiative/reaction specialty family.
- targeted `hellish_rebuke_resolve` / initiative behavior suites plus adjacent allowlist/contract checks.
