...
def normalize_configured_copilot_login(value: str | None) -> tuple[str, bool]:
    ...
    canonical = _LEGACY_LOGIN_ALIASES.get(normalized, normalized) or normalize_login(
        DOCUMENTED_COPILOT_ASSIGNEE_LOGIN
    )
...
def is_copilot_identity(login: str | None, configured_login: str | None) -> bool:
    ...
    return bool(normalized_login) and normalized_login == configured_canonical
