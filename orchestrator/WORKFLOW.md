...
- successful check state is persisted per PR head SHA (`last_successful_checks_head_sha`); later PR events reuse that result only when the current head SHA matches, so stale green checks are never carried forward to newer commits
