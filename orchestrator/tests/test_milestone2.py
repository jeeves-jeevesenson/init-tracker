...
class OpenAIPlanningSchemaTests(unittest.TestCase):
    ...
    def test_merge_audit_requires_meaningful_transition_before_heavy_recall(self):
        with tempfile.TemporaryDirectory() as td:
            os.environ["DATABASE_URL"] = f"sqlite:///{Path(td) / 'orchestrator.db'}"
            os.environ["GH_WEBHOOK_SECRET"] = "test-gh-secret"
            os.environ["GITHUB_API_TOKEN"] = "dummy-token"
            os.environ["GITHUB_DISPATCH_USER_TOKEN"] = "dispatch-token"
            main, _ = _reload_orchestrator_modules()
            issue_payload = {"action": "opened", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "issue": {"number": 9625, "node_id": "I_9625", "title": "Meaningful gate", "body": "docs", "labels": [{"name": "agent:task"}]}}
            approve_payload = {"action": "created", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "issue": {"number": 9625}, "comment": {"body": "/approve"}}
            pr_open_payload = {"action": "opened", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "pull_request": {"id": 962501, "number": 96251, "title": "Docs", "body": "Closes #9625", "html_url": "https://github.com/jeeves-jeevesenson/init-tracker/pull/96251", "updated_at": "2026-01-01T00:00:00Z", "draft": False, "mergeable": True, "mergeable_state": "clean", "head": {"sha": "sha-9625"}, "requested_reviewers": []}}
            pr_edit_payload = {"action": "edited", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "pull_request": {"id": 962501, "number": 96251, "title": "Docs", "body": "Closes #9625", "html_url": "https://github.com/jeeves-jeevesenson/init-tracker/pull/96251", "updated_at": "2026-01-01T00:10:00Z", "draft": False, "mergeable": True, "mergeable_state": "clean", "head": {"sha": "sha-9625"}, "requested_reviewers": []}}
            workflow_payload = {"action": "completed", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "workflow_run": {"id": 962502, "name": "CI", "status": "completed", "conclusion": "success", "head_sha": "sha-9625", "html_url": "https://github.com/jeeves-jeevesenson/init-tracker/actions/runs/962502", "pull_requests": [{"number": 96251}]}}
            with patch("orchestrator.app.tasks.plan_task_packet", return_value={"objective": "Docs", "scope": ["docs"], "non_goals": [], "acceptance_criteria": ["docs"], "validation_guidance": [], "implementation_brief": "docs"}), \
                 patch("orchestrator.app.tasks.dispatch_task_to_github_copilot", return_value=DispatchResult(attempted=True, accepted=True, manual_required=False, state="accepted", summary="ok")), \
                 patch("orchestrator.app.tasks.inspect_pull_request", return_value=PullRequestInspection(ok=True, changed_files=1, commits=1, draft=False, state="open", merged=False, mergeable=True, mergeable_state="clean", summary="ok")), \
                 patch("orchestrator.app.tasks.list_pull_request_file_details", return_value=([{"filename": "docs/slice.md", "status": "modified", "additions": 1, "deletions": 0, "patch": "+slice"}], "ok")), \
                 patch("orchestrator.app.tasks.list_pull_request_reviews", return_value=([], "ok")), \
                 patch("orchestrator.app.tasks.list_pull_request_review_comments", return_value=([], "ok")), \
                 patch("orchestrator.app.tasks.summarize_merge_audit", return_value={"merge_audit_artifact": {"decision": "wait", "confidence": 0.7, "doc_only": True, "safe_to_merge": False, "requires_followup": True, "summary": ["hold"], "findings": [], "merge_rationale": "", "escalation_reason": "", "review_scope": ["patch"]}}) as mocked_merge_audit:
                with TestClient(main.app) as client:
                    _post_github_local(client, secret="test-gh-secret", delivery="d-9625-open", event="issues", payload=issue_payload)
                    _post_github_local(client, secret="test-gh-secret", delivery="d-9625-approve", event="issue_comment", payload=approve_payload)
                    _post_github_local(client, secret="test-gh-secret", delivery="d-9625-pr-open", event="pull_request", payload=pr_open_payload)
                    _post_github_local(client, secret="test-gh-secret", delivery="d-9625-wf", event="workflow_run", payload=workflow_payload)
                    first_count = mocked_merge_audit.call_count
                    _post_github_local(client, secret="test-gh-secret", delivery="d-9625-pr-edit", event="pull_request", payload=pr_edit_payload)
                    self.assertEqual(mocked_merge_audit.call_count, first_count)
                    state = client.get("/tasks").json()["tasks"][0]["latest_run"]["governor_state"]
                    self.assertEqual(state.get("last_heavy_block_reason"), "heavy_review_gated_no_meaningful_transition")

    def test_governor_context_trims_umbrella_criteria_on_repeated_calls(self):
        with tempfile.TemporaryDirectory() as td:
            os.environ["DATABASE_URL"] = f"sqlite:///{Path(td) / 'orchestrator.db'}"
            os.environ["GH_WEBHOOK_SECRET"] = "test-gh-secret"
            contexts: list[dict] = []
            main, _ = _reload_orchestrator_modules()
            issue_payload = {"action": "opened", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "issue": {"number": 9626, "node_id": "I_9626", "title": "Umbrella", "body": "large umbrella body", "labels": [{"name": "agent:task"}]}}
            approve_payload = {"action": "created", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "issue": {"number": 9626}, "comment": {"body": "/approve"}}

            def _capture_ctx(*, settings, update_context, previous_response_id=None, model=None, stage="continuation_audit"):
                contexts.append(json.loads(update_context))
                return {"governor_artifact": {"decision": "wait", "summary": ["ok"], "revision_requests": [], "escalation_reason": ""}}

            with patch("orchestrator.app.tasks.plan_task_packet", return_value={"objective": "umbrella", "scope": [], "non_goals": [], "acceptance_criteria": ["umbrella milestone 1", "umbrella milestone 2"], "validation_guidance": [], "implementation_brief": "umbrella", "recommended_scope_class": "broad"}), \
                 patch("orchestrator.app.tasks.dispatch_task_to_github_copilot", return_value=DispatchResult(attempted=True, accepted=True, manual_required=False, state="accepted", summary="ok")), \
                 patch("orchestrator.app.tasks.inspect_pull_request", return_value=PullRequestInspection(ok=True, changed_files=1, commits=1, draft=False, state="open", merged=False, mergeable=False, mergeable_state="blocked", summary="ok")), \
                 patch("orchestrator.app.tasks.list_pull_request_file_details", return_value=([{"filename": "docs/slice.md", "status": "modified", "additions": 1, "deletions": 0, "patch": "+slice"}], "ok")), \
                 patch("orchestrator.app.tasks.list_pull_request_reviews", return_value=([], "ok")), \
                 patch("orchestrator.app.tasks.list_pull_request_review_comments", return_value=([], "ok")), \
                 patch("orchestrator.app.tasks.summarize_governor_update", side_effect=_capture_ctx), \
                 patch("orchestrator.app.tasks.notify_discord"):
                with TestClient(main.app) as client:
                    _post_github_local(client, secret="test-gh-secret", delivery="d-9626-open", event="issues", payload=issue_payload)
                    _post_github_local(client, secret="test-gh-secret", delivery="d-9626-approve", event="issue_comment", payload=approve_payload)
                    first = {"action": "opened", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "pull_request": {"id": 962601, "number": 96261, "title": "Slice", "body": "Closes #9626", "html_url": "https://github.com/jeeves-jeevesenson/init-tracker/pull/96261", "updated_at": "2026-01-01T00:00:00Z", "draft": False, "mergeable": False, "mergeable_state": "blocked", "head": {"sha": "sha-9626-a"}, "requested_reviewers": []}}
                    _post_github_local(client, secret="test-gh-secret", delivery="d-9626-pr-open", event="pull_request", payload=first)
                    second = json.loads(json.dumps(first))
                    second["action"] = "synchronize"
                    second["pull_request"]["head"]["sha"] = "sha-9626-b"
                    _post_github_local(client, secret="test-gh-secret", delivery="d-9626-pr-sync", event="pull_request", payload=second)
                    self.assertGreaterEqual(len(contexts), 2)
                    self.assertIn("program_acceptance_criteria", contexts[0])
                    self.assertNotIn("program_acceptance_criteria", contexts[-1])
                    self.assertTrue(contexts[-1].get("umbrella_context_trimmed"))
    ...
    def test_review_batch_artifact_normalizes_contradictory_should_trigger_flag(self):
        artifact = openai_review._validate_review_batch_artifact(
            {
                "summary": ["batch"],
                "batched_items": ["Fix null-check handling."],
                "comment_body": "@copilot Please apply the fix.",
                "should_trigger_copilot": False,
            }
        )
        self.assertTrue(artifact["should_trigger_copilot"])
        self.assertEqual(artifact["batched_items"], ["Fix null-check handling."])

    def test_review_batch_no_trigger_state_is_truthful_when_followup_not_posted(self):
        with tempfile.TemporaryDirectory() as td:
            os.environ["DATABASE_URL"] = f"sqlite:///{Path(td) / 'orchestrator.db'}"
            os.environ["GH_WEBHOOK_SECRET"] = "test-gh-secret"
            main, _ = _reload_orchestrator_modules()
            issue_payload = {"action": "opened", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "issue": {"number": 998, "node_id": "I_998", "title": "No trigger", "body": "test", "labels": [{"name": "agent:task"}]}}
            approve_payload = {"action": "created", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "issue": {"number": 998}, "comment": {"body": "/approve"}}
            reviews_payload = [{"state": "COMMENTED", "body": "Please follow up.", "user": {"login": DOCUMENTED_COPILOT_ASSIGNEE_LOGIN}}]
            review_comments_payload = [{"body": "Address ordering bug.", "user": {"login": DOCUMENTED_COPILOT_ASSIGNEE_LOGIN}, "commit_id": "sha-998-a", "original_commit_id": "sha-998-a"}]
            with patch("orchestrator.app.tasks.plan_task_packet", return_value={"objective": "No trigger", "scope": [], "non_goals": [], "acceptance_criteria": [], "validation_guidance": [], "implementation_brief": "x"}), \
                 patch("orchestrator.app.tasks.dispatch_task_to_github_copilot", return_value=DispatchResult(attempted=True, accepted=True, manual_required=False, state="accepted", summary="ok")), \
                 patch("orchestrator.app.tasks.inspect_pull_request", return_value=PullRequestInspection(ok=True, changed_files=1, commits=1, draft=False, state="open", merged=False, mergeable=False, mergeable_state="blocked", summary="ok")), \
                 ...
                 patch("orchestrator.app.tasks.list_issue_comments", return_value=([], "ok")), \
                 patch("orchestrator.app.tasks.summarize_copilot_review_batch", return_value={"review_batch_artifact": {"summary": ["skip"], "batched_items": [], "comment_body": "", "should_trigger_copilot": False}}), \
                 patch("orchestrator.app.tasks.post_copilot_follow_up_comment", return_value=(True, "ok")) as mocked_comment, \
                 patch("orchestrator.app.tasks.notify_discord"):
                with TestClient(main.app) as client:
                    _post_github_local(client, secret="test-gh-secret", delivery="d-998-open", event="issues", payload=issue_payload)
                    _post_github_local(client, secret="test-gh-secret", delivery="d-998-approve", event="issue_comment", payload=approve_payload)
                    opened = {"action": "opened", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "pull_request": {"id": 99801, "number": 9981, "title": "Slice", "body": "Closes #998", "html_url": "https://github.com/jeeves-jeevesenson/init-tracker/pull/9981", "updated_at": "2026-01-01T00:00:00Z", "draft": False, "mergeable": False, "mergeable_state": "blocked", "head": {"sha": "sha-998-a"}, "requested_reviewers": []}}
                    _post_github_local(client, secret="test-gh-secret", delivery="d-998-pr-open", event="pull_request", payload=opened)
                    state = client.get("/tasks").json()["tasks"][0]["latest_run"]["governor_state"]
                    self.assertEqual(mocked_comment.call_count, 0)
                    self.assertEqual(state.get("last_governor_decision"), "review_batch_no_trigger")
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch("orchestrator.app.tasks.plan_task_packet", return_value={"objective": "Packet dedupe", "scope": [], "non_goals": [], "acceptance_criteria": [], "validation_guidance": [], "implementation_brief": "x"}), \
                ...
                with TestClient(main.app) as client:
                    ...
                    self.assertEqual(state.get("last_governor_decision"), "review_batch_trigger_waiting")

    def test_current_head_findings_block_merge_until_followup_reverify_state(self):
        with tempfile.TemporaryDirectory() as td:
            os.environ["DATABASE_URL"] = f"sqlite:///{Path(td) / 'orchestrator.db'}"
            os.environ["GH_WEBHOOK_SECRET"] = "test-gh-secret"
            main, _ = _reload_orchestrator_modules()
            issue_payload = {"action": "opened", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "issue": {"number": 999, "node_id": "I_999", "title": "Block merge", "body": "test", "labels": [{"name": "agent:task"}]}}
            approve_payload = {"action": "created", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "issue": {"number": 999}, "comment": {"body": "/approve"}}
            reviews_payload = [{"state": "COMMENTED", "body": "Please follow up.", "user": {"login": DOCUMENTED_COPILOT_ASSIGNEE_LOGIN}}]
            review_comments_payload = [{"body": "Address ordering bug.", "user": {"login": DOCUMENTED_COPILOT_ASSIGNEE_LOGIN}, "commit_id": "sha-999-a", "original_commit_id": "sha-999-a"}]
            with patch("orchestrator.app.tasks.plan_task_packet", return_value={"objective": "Block merge", "scope": [], "non_goals": [], "acceptance_criteria": [], "validation_guidance": [], "implementation_brief": "x"}), \
                 patch("orchestrator.app.tasks.dispatch_task_to_github_copilot", return_value=DispatchResult(attempted=True, accepted=True, manual_required=False, state="accepted", summary="ok")), \
                 patch("orchestrator.app.tasks.inspect_pull_request", return_value=PullRequestInspection(ok=True, changed_files=1, commits=1, draft=False, state="open", merged=False, mergeable=True, mergeable_state="clean", summary="ok")), \
                 patch("orchestrator.app.tasks.list_pull_request_file_details", return_value=([{"filename": "orchestrator/app/tasks.py", "status": "modified", "additions": 1, "deletions": 0, "patch": "+x"}], "ok")), \
                 patch("orchestrator.app.tasks.list_pull_request_reviews", return_value=(reviews_payload, "ok")), \
                 patch("orchestrator.app.tasks.list_pull_request_review_comments", return_value=(review_comments_payload, "ok")), \
                 patch("orchestrator.app.tasks.list_issue_comments", return_value=([], "ok")), \
                 patch("orchestrator.app.tasks.summarize_copilot_review_batch", return_value={"review_batch_artifact": {"summary": ["one"], "batched_items": ["fix"], "comment_body": "@copilot please fix finding", "should_trigger_copilot": True}}), \
                 patch("orchestrator.app.tasks.post_copilot_follow_up_comment", return_value=(True, "ok")) as mocked_comment, \
                 patch("orchestrator.app.tasks.merge_pr") as mocked_merge, \
                 patch("orchestrator.app.tasks.notify_discord"):
                with TestClient(main.app) as client:
                    _post_github_local(client, secret="test-gh-secret", delivery="d-999-open", event="issues", payload=issue_payload)
                    _post_github_local(client, secret="test-gh-secret", delivery="d-999-approve", event="issue_comment", payload=approve_payload)
                    opened = {"action": "opened", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "pull_request": {"id": 99901, "number": 9991, "title": "Slice", "body": "Closes #999", "html_url": "https://github.com/jeeves-jeevesenson/init-tracker/pull/9991", "updated_at": "2026-01-01T00:00:00Z", "draft": False, "mergeable": True, "mergeable_state": "clean", "head": {"sha": "sha-999-a"}, "requested_reviewers": []}}
                    _post_github_local(client, secret="test-gh-secret", delivery="d-999-pr-open", event="pull_request", payload=opened)
                    state = client.get("/tasks").json()["tasks"][0]["latest_run"]["governor_state"]
                    self.assertEqual(mocked_comment.call_count, 1)
                    mocked_merge.assert_not_called()
                    self.assertEqual(state.get("last_governor_decision"), "review_batch_trigger_posted")
                    self.assertEqual(state.get("current_head_review_state"), "awaiting_current_head_reverify")
                    self.assertTrue(bool(state.get("waiting_for_revision_push")))
    ...
    def test_review_state_rehydration_recovers_missed_webhook_findings_and_dedupes(self):
        with tempfile.TemporaryDirectory() as td:
            os.environ["DATABASE_URL"] = f"sqlite:///{Path(td) / 'orchestrator.db'}"
            os.environ["GH_WEBHOOK_SECRET"] = "test-gh-secret"
            main, _ = _reload_orchestrator_modules()
            issue_payload = {"action": "opened", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "issue": {"number": 994, "node_id": "I_994", "title": "Recover findings", "body": "test", "labels": [{"name": "agent:task"}]}}
            approve_payload = {"action": "created", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "issue": {"number": 994}, "comment": {"body": "/approve"}}
            reviews_side_effect = [
                ([], "ok"),
                ([], "ok"),
                ([{"state": "CHANGES_REQUESTED", "body": "Fix validation flow.", "user": {"login": DOCUMENTED_COPILOT_ASSIGNEE_LOGIN}}], "ok"),
                ([{"state": "CHANGES_REQUESTED", "body": "Fix validation flow.", "user": {"login": DOCUMENTED_COPILOT_ASSIGNEE_LOGIN}}], "ok"),
                ([{"state": "CHANGES_REQUESTED", "body": "Fix validation flow.", "user": {"login": DOCUMENTED_COPILOT_ASSIGNEE_LOGIN}}], "ok"),
                ([{"state": "CHANGES_REQUESTED", "body": "Fix validation flow.", "user": {"login": DOCUMENTED_COPILOT_ASSIGNEE_LOGIN}}], "ok"),
            ]
            review_comments_side_effect = [
                ([], "ok"),
                ([], "ok"),
                ([{"body": "Guard empty payload in parser.", "user": {"login": DOCUMENTED_COPILOT_ASSIGNEE_LOGIN}, "commit_id": "sha-994-a", "original_commit_id": "sha-994-a"}], "ok"),
                ([{"body": "Guard empty payload in parser.", "user": {"login": DOCUMENTED_COPILOT_ASSIGNEE_LOGIN}, "commit_id": "sha-994-a", "original_commit_id": "sha-994-a"}], "ok"),
                ([{"body": "Guard empty payload in parser.", "user": {"login": DOCUMENTED_COPILOT_ASSIGNEE_LOGIN}, "commit_id": "sha-994-a", "original_commit_id": "sha-994-a"}], "ok"),
                ([{"body": "Guard empty payload in parser.", "user": {"login": DOCUMENTED_COPILOT_ASSIGNEE_LOGIN}, "commit_id": "sha-994-a", "original_commit_id": "sha-994-a"}], "ok"),
            ]
            with patch("orchestrator.app.tasks.plan_task_packet", return_value={"objective": "Recover", "scope": [], "non_goals": [], "acceptance_criteria": [], "validation_guidance": [], "implementation_brief": "x"}), \
                 patch("orchestrator.app.tasks.dispatch_task_to_github_copilot", return_value=DispatchResult(attempted=True, accepted=True, manual_required=False, state="accepted", summary="ok")), \
                 patch("orchestrator.app.tasks.inspect_pull_request", return_value=PullRequestInspection(ok=True, changed_files=1, commits=1, draft=False, state="open", merged=False, mergeable=False, mergeable_state="blocked", summary="ok")), \
                 patch("orchestrator.app.tasks.list_pull_request_file_details", return_value=([{"filename": "orchestrator/app/tasks.py", "status": "modified", "additions": 2, "deletions": 0, "patch": "+x"}], "ok")), \
                 patch("orchestrator.app.tasks.list_pull_request_reviews", side_effect=reviews_side_effect), \
                 patch("orchestrator.app.tasks.list_pull_request_review_comments", side_effect=review_comments_side_effect), \
                 patch("orchestrator.app.tasks.list_issue_comments", return_value=([], "ok")), \
                 patch("orchestrator.app.tasks.summarize_copilot_review_batch", return_value={"review_batch_artifact": {"summary": ["one"], "batched_items": ["fix"], "comment_body": "@copilot please fix parser", "should_trigger_copilot": True}}), \
                 patch("orchestrator.app.tasks.post_copilot_follow_up_comment", return_value=(True, "ok")) as mocked_comment, \
                 patch("orchestrator.app.tasks.notify_discord"):
                with TestClient(main.app) as client:
                    _post_github_local(client, secret="test-gh-secret", delivery="d-994-open", event="issues", payload=issue_payload)
                    _post_github_local(client, secret="test-gh-secret", delivery="d-994-approve", event="issue_comment", payload=approve_payload)
                    pr_payload = {"action": "opened", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "pull_request": {"id": 99401, "number": 9941, "title": "Slice", "body": "Closes #994", "html_url": "https://github.com/jeeves-jeevesenson/init-tracker/pull/9941", "updated_at": "2026-01-01T00:00:00Z", "draft": False, "mergeable": False, "mergeable_state": "blocked", "head": {"sha": "sha-994-a"}, "requested_reviewers": []}}
                    _post_github_local(client, secret="test-gh-secret", delivery="d-994-pr-open", event="pull_request", payload=pr_payload)
                    edited = json.loads(json.dumps(pr_payload))
                    edited["action"] = "edited"
                    edited["pull_request"]["updated_at"] = "2026-01-01T00:05:00Z"
                    _post_github_local(client, secret="test-gh-secret", delivery="d-994-pr-edit-1", event="pull_request", payload=edited)
                    _post_github_local(client, secret="test-gh-secret", delivery="d-994-pr-edit-2", event="pull_request", payload=edited)
                    state = client.get("/tasks").json()["tasks"][0]["latest_run"]["governor_state"]
                    self.assertEqual(mocked_comment.call_count, 1)
                    self.assertTrue(bool(state.get("waiting_for_revision_push")))
                    self.assertEqual(state.get("outstanding_followup_status"), "open")
                    self.assertIn(
                        state.get("last_governor_decision"),
                        {"review_batch_trigger_posted", "review_batch_trigger_waiting"},
                    )

    def test_new_head_requires_reverify_instead_of_false_clear_after_followup_packet(self):
        with tempfile.TemporaryDirectory() as td:
            os.environ["DATABASE_URL"] = f"sqlite:///{Path(td) / 'orchestrator.db'}"
            os.environ["GH_WEBHOOK_SECRET"] = "test-gh-secret"
            main, _ = _reload_orchestrator_modules()
            issue_payload = {"action": "opened", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "issue": {"number": 993, "node_id": "I_993", "title": "Reverify head", "body": "test", "labels": [{"name": "agent:task"}]}}
            approve_payload = {"action": "created", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "issue": {"number": 993}, "comment": {"body": "/approve"}}
            with patch("orchestrator.app.tasks.plan_task_packet", return_value={"objective": "Reverify", "scope": [], "non_goals": [], "acceptance_criteria": [], "validation_guidance": [], "implementation_brief": "x"}), \
                 patch("orchestrator.app.tasks.dispatch_task_to_github_copilot", return_value=DispatchResult(attempted=True, accepted=True, manual_required=False, state="accepted", summary="ok")), \
                 patch("orchestrator.app.tasks.inspect_pull_request", return_value=PullRequestInspection(ok=True, changed_files=1, commits=1, draft=False, state="open", merged=False, mergeable=True, mergeable_state="clean", summary="ok")), \
                 patch("orchestrator.app.tasks.list_pull_request_file_details", return_value=([{"filename": "orchestrator/app/tasks.py", "status": "modified", "additions": 2, "deletions": 0, "patch": "+x"}], "ok")), \
                 patch("orchestrator.app.tasks.list_pull_request_reviews", side_effect=[([{"state": "CHANGES_REQUESTED", "body": "Fix null check.", "user": {"login": DOCUMENTED_COPILOT_ASSIGNEE_LOGIN}}], "ok"), ([], "ok"), ([], "ok")]), \
                 patch("orchestrator.app.tasks.list_pull_request_review_comments", side_effect=[([], "ok"), ([], "ok"), ([], "ok")]), \
                 patch("orchestrator.app.tasks.list_issue_comments", return_value=([], "ok")), \
                 patch("orchestrator.app.tasks.summarize_copilot_review_batch", return_value={"review_batch_artifact": {"summary": ["one"], "batched_items": ["fix"], "comment_body": "@copilot fix null check", "should_trigger_copilot": True}}), \
                 patch("orchestrator.app.tasks.post_copilot_follow_up_comment", return_value=(True, "ok")), \
                 patch("orchestrator.app.tasks.notify_discord"):
                with TestClient(main.app) as client:
                    _post_github_local(client, secret="test-gh-secret", delivery="d-993-open", event="issues", payload=issue_payload)
                    _post_github_local(client, secret="test-gh-secret", delivery="d-993-approve", event="issue_comment", payload=approve_payload)
                    opened = {"action": "opened", "repository": {"full_name": "jeeves-jeevesenson/init-tracker"}, "pull_request": {"id": 99301, "number": 9931, "title": "Slice", "body": "Closes #993", "html_url": "https://github.com/jeeves-jeevesenson/init-tracker/pull/9931", "updated_at": "2026-01-01T00:00:00Z", "draft": False, "mergeable": True, "mergeable_state": "clean", "head": {"sha": "sha-993-a"}, "requested_reviewers": []}}
                    _post_github_local(client, secret="test-gh-secret", delivery="d-993-pr-open", event="pull_request", payload=opened)
                    synced = json.loads(json.dumps(opened))
                    synced["action"] = "synchronize"
                    synced["pull_request"]["updated_at"] = "2026-01-01T00:10:00Z"
                    synced["pull_request"]["head"]["sha"] = "sha-993-b"
                    _post_github_local(client, secret="test-gh-secret", delivery="d-993-pr-sync", event="pull_request", payload=synced)
                    state = client.get("/tasks").json()["tasks"][0]["latest_run"]["governor_state"]
                    self.assertEqual(state.get("outstanding_followup_status"), "pending_current_head_verification")
                    self.assertFalse(bool(state.get("waiting_for_revision_push")))
                    self.assertEqual(state.get("current_head_review_state"), "current_head_findings_cleared")
